...
export async function POST(
  ...
  // Verificar que el wish existe y pertenece al usuario
  const wish = await prisma.wish.findFirst({
    where: { id, userId: session.user.id },
  });

  if (!wish) {
    return NextResponse.json({ error: "Deseo no encontrado" }, { status: 404 });
  }
  ...
  // Usar precio del wish si no se proporciona uno nuevo
  const finalPrice = price !== undefined ? price : wish.price;
  const finalNotes = notes !== undefined ? notes : wish.notes;
  ...
  // Crear el record y eliminar el wish en una transacción
